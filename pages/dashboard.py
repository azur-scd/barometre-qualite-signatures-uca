...
df_bsi_publis_uniques = pd.read_sql(
    f'select annee_pub,synthese_mention_adresse_norm from bsi_publis_uniques_{publis_last_obs_date}', dbEngine)
...
df_bsi_all_by_mention = pd.read_sql(
    f'select annee_pub,affiliation_name,mention_adresse_norm from bsi_all_by_mention_adresse_{publis_last_obs_date}', dbEngine)
        ...
        dbc.Row(
            [
                dbc.Col([get_slider_range("slider-pie-regroup"),
                        dcc.Graph(id="pie-regroup")], style={"borderRight": "1px solid #7191b3"},width=5),
                dbc.Col([dbc.RadioItems(
                    ...
                    dcc.Graph(id="barchart-regroup")], width=7),
            ]
        ),
    ],
...
row_widgets_detail = html.Div([
    dbc.Row([
        ...
        dbc.Col(dbc.RadioItems(
                    options=[
                        {"label": "Valeur absolue", "value": "qte"},
                         {"label": "Valeur absolue en échelle logarithmique", "value": "log"},
                        {"label": "Pourcentage", "value": "percent"},
                    ],
                    inline=True,
                    value="qte",
    ...
    dbc.Row(
        dbc.Col(dcc.Graph(id="barchart-detail"), width=12))
],
className="p-3 bg-light rounded-3")
...
layout = [html.H3("Analyse de la qualité des signatures au niveau des publications", className="text-center"),
          ...
          row_widgets_regroup,
          html.Hr(),
          html.H3("Analyse de la qualité des signatures au niveau des mentions d'affiliation", className="text-center"),
...
def update_pie_regroup(slider_pie_regroup):
    ...
    fig = px.pie(filtered_data, names='synthese_mention_adresse_norm', color='synthese_mention_adresse_norm',
                 color_discrete_map=colors, hole=0.7, title="Ventilation globale des types de mention d'adresse")
    return fig
...
def update_detail_section(radio_detail_datatype,slider_barchart_detail):
    ...
    crosstab_simple= fn.get_crosstab_simple(filtered_data, "affiliation_name", "mention_adresse_norm").sort_values('Total', ascending=False).reset_index()
    crosstab_percent = fn.get_crosstab_percent(filtered_data, "affiliation_name", "mention_adresse_norm")
    if (radio_detail_datatype == 'qte') | (radio_detail_datatype == 'log'):
        ...
        if radio_detail_datatype == 'qte':
            fig = px.bar(data_no_margins, x='affiliation_name_wrapped',y=chart_cols, color_discrete_map=colors, height=700)
        if radio_detail_datatype == 'log':
            fig = px.bar(data_no_margins, x='affiliation_name_wrapped',y=chart_cols, color_discrete_map=colors, log_y=True, height=700)
        fig.update_traces(textposition='inside', texttemplate="%{value}")
